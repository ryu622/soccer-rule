"""evolve_skeleton_v4.py の実行結果を可視化し、held-out試合で最終評価する。
...
from evolve_skeleton_v4 import (
    DATA_PATH,
    EVOLVE_SET_MATCHES,
...
def latest_run_dir() -> str:
    dirs = sorted(glob.glob("evolution_runs_v4/*"))
    if not dirs:
        raise FileNotFoundError("evolution_runs_v4/ にディレクトリが見つかりません")
    return dirs[-1]
...
def plot_progress(df: pd.DataFrame, run_config: dict, out_path: str) -> None:
    ...
    ax.set_title("進化計算v4の世代推移(reflection+誤分類事例・破線=移住)")
    ax.legend()
    fig.tight_layout()
...
def main() -> None:
    ...
    run_dir = f"evolution_runs_v4/{args.run_id}" if args.run_id else latest_run_dir()
    ...
    plot_progress(df, run_config, f"{FIG_DIR}/evolution_v4_progress.png")
